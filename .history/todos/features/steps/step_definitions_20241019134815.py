...
@when("the todo with title '{todo_title}' is updated with title '{new_title}' and optional fields doneStatus '{doneStatus}' and description '{new_description}'")
def step_impl(context, todo_title, new_title, doneStatus=None, new_description=None):
    if not context.api_is_running:
        return
    ...
    if doneStatus is not None:
        data["doneStatus"] = doneStatus.lower() == "true"
    ...
    context.response = requests.put(f"{API_URL}/todos/{todo_id}", json=data)
    assert context.response.status_code == 200, f"Failed to update todo with id '{todo_id}'."
