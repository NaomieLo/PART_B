...
def step_impl(context):
    ...
    context.response = requests.get(f"{API_URL}/todos/9999/projects")

    assert (
        context.response.status_code == 40
...
@then("the response contains empty list for '{key}' with id '{id_value}'")
def step_impl(context, key, id_value):
    if not context.api_is_running:
        return

    # Make a GET request to retrieve the resource with the specific id
    context.response = requests.get(f"{API_URL}/todos/{id_value}/{key}")
    ...
    if context.response.text:
        try:
            ...
            raise AssertionError(
                f"Expected valid JSON response, but got: {context.response.text}"
            )
    else:
        # If the response body is empty, we treat it as an empty list
        response_data = []

    # Ensure the key exists in the response or handle the empty body case
    assert (
        response_data is not None
    ), f"Key '{key}' not found in the response for ID '{id_value}'."

    # Ensure the key corresponds to a list
    assert isinstance(
        response_data, list
    ), f"Expected a list for key '{key}', but found {type(response_data).__name__}."
    ...
    assert (
        len(response_data) == 0
    ), f"Expected an empty list for key '{key}' with ID '{id_value}', but found some items: {response_data}"
