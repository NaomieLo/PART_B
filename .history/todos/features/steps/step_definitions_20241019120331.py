...
def step_impl(context, todo_title):
    ...
    # Step 1: Get the todo ID using the helper function
    todo_id = get_todo_id(todo_title)

    # Step 2: Retrieve all project items (tasksof) associated with the todo
    tasksof_response = context.client.get(f"/todos/{todo_id}/tasksof")
    tasks = tasksof_response.json().get("projects", [])

    # Step 3: Iterate through each project item and delete it
    for task in tasks:
        task_id = task["id"]
        # Delete each task linked to the todo
        context.client.delete(f"/todos/{todo_id}/tasksof/{task_id}")

    # Now the todo has no task instances
