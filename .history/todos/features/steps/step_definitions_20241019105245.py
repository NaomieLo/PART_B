...
@when(
    "the user makes a HEAD request to /todos/:id/categories for the todo with title '{todo_title}'"
)
def step_impl(context, todo_title):
    if not context.api_is_running:
        return
    ...
    context.response = requests.head(f"{API_URL}/todos/{todo_id}/categories")

    assert context.response.status_code == 200, f"Failed to get HEAD request"
