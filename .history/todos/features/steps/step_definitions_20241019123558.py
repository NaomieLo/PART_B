...
def step_impl(context):
    ...
    # Make the GET request for a non-existent todo ID 9999
...
def step_impl(context, key, id_value):
    ...
    if context.response.text:
        ...
        # If the response body is empty, we can't check for an empty list
        raise AssertionError(f"Expected a valid JSON response, but the body is empty.")
