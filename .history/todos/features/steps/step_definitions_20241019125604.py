...
def step_impl(context, todo_id, key):
    ...
    response = requests.get(f"{API_URL}/todos/930129310293810928/tasksof")

    # Parse the response JSON and get the key value
    response_data = response.json().get(key, None)

    # Ensure that the key exists in the response
    assert response_data is not None, f"Key '{key}' not found in the response for todo with ID '{todo_id}'."
