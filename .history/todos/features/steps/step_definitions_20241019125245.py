...
@then("the response for todo with id '{todo_id}' contains an empty list for '{key}'")
def step_impl(context, todo_id, key):
    if not context.api_is_running:
        return

    # Make a new GET request to retrieve the resource with the specific id
    response = requests.get(f"{API_URL}/todos/{todo_id}/tasksof")
    ...
    # Ensure that the response has the expected key and the value is an empty list
    assert key in response_data, f"Key '{key}' not found in the response for todo with ID '{todo_id}'."
    
    # Ensure the key corresponds to an empty list
    assert isinstance(response_data[key], list), f"Expected a list for key '{key}', but found {type(response_data[key]).__name__}."
    
    # Ensure the list is empty
    assert len(response_data[key]) == 0, f"Expected an empty list for key '{key}', but found: {response_data[key]}"
