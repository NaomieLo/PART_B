...
def restore_initial_state(context):
    ...
    for initial_category in context.initial_categories:
     ...
     # delete any new instances of project 
    for project in projects:
        if project["title"] not in [initial_project["title"] for initial_project in context.initial_projects.values()]:
            delete_response = requests.delete(f"{API_URL}/projects/{project['id']}")
            assert delete_response.status_code == 200, f"Failed to delete project with title {project['title']}"

    # re post or fix existing for todos 
    for initial_project in context.initial_projects.values():
    existing_project = next(
        (project for project in projects if project["title"] == initial_project["title"]),
        None,
    )

    # Update if it still exists but its properties differ
    if existing_project:
        if (
            existing_project.get("description", "") != initial_project["description"]
            or existing_project.get("active", "") != initial_project["active"]
            or existing_project.get("completed", "") != initial_project["completed"]
        ):
            update_data = {
            ...
            assert update_response.status_code == 200, f"Failed to update project with title {initial_project['title']}"
    
    # Repost if it was deleted
    else:
        data = {
            "title": initial_project["title"],
            "description": initial_project["description"],
        ...
        create_response = requests.post(f"{API_URL}/projects", json=data)
