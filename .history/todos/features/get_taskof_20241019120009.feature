Feature: Update Todo
  ...
  Scenario: Retrieve project items for a todo with no tasksof
    Given the todo with the title 'Workout' already has no tasks instances 
    When the user makes a GET request to /todos/:id/tasksof for the todo with title 'Workout'
    Then the status code 200 will be received
