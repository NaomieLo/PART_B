Feature: Update Todo
  ...
  Scenario: Successfully retrieve project items (tasksof) for a todo
    ...
    And the response will include the project with title 'Morning Run' and 'Evening Stretch'
  ...
  Scenario: Retrieve project items for a todo with no tasksof
    ...
    Then the status code 200 will be received
    And the response of the todo with title 'Workout' will include an empty array in the body
