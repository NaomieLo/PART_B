Feature: Update Project
  ...
  Background:
    ...
    And there is an existing project with title "Old Project Title"

  Scenario: Successfully update a project with a new title and description
    When the user updates the project with title "Old Project Title" to have title "Updated Title" and description "Updated Description"
    Then the status code 200 will be received
    And the project with title "Updated Title" should exist in the database

  Scenario: Update a project with only a new title
    When the user updates the project with title "Old Project Title" to have title "Another Title"
    Then the status code 200 will be received
